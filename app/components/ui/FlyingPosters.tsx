...
function AutoBind(
  ...
  const getAllProperties = (object: unknown) => {
  ...
  for (const [object, key] of getAllProperties((self as { constructor: { prototype: object } }).constructor.prototype)) {
    if (key === "constructor" || !filter(key)) continue;
    const descriptor = Reflect.getOwnPropertyDescriptor(object, key);
    if (descriptor && typeof descriptor.value === "function") {
      (self as Record<string, unknown>)[String(key)] = (descriptor.value as (...args: unknown[]) => unknown).bind(self);
    }
  }
}
...
class CanvasScene {
  ...
  update() {
    ...
    if (this.medias) this.medias.forEach((media: Media) => media.update(this.scroll));
...
export default function FlyingPosters({
  ...
  }, [cameraFov, cameraZ, distortion, items, planeHeight, planeWidth, scrollEase]);
